...
def simulate_team_seasons(df, trace, nsims=1000, metric='score',
                          K=1, burnin=1000):
    dfs = []
    for i in range(nsims):
        season = simulate_team_season(df, trace, metric=metric,
                                      K=K, burnin=burnin)
...
def simulate_team_season(df, trace, metric='scores', K=1, burnin=10000):
    ...
    season['home_theta'] = season.apply(lambda x:
                                        np.exp(x['intercept'] +
                                               x['home'] +
                                               x['att_home'] +
                                               x['def_away']), axis=1)

    season['away_theta'] = season.apply(lambda x:
                                        np.exp(x['intercept'] +
                                               x['att_away'] +
                                               x['def_home']), axis=1)

    season[home_metric] = season.apply(lambda x:
                                       K*np.random.poisson(x['home_theta']),
                                       axis=1)
    season[away_metric] = season.apply(lambda x:
                                       K*np.random.poisson(x['away_theta']),
                                       axis=1)
    return season


def simulate_seasons(df, trace, teams, nsims=1000, metric='score',
                     K=1, burnin=1000):
    dfs = []
    for i in range(nsims):
        season = simulate_season(df, trace, metric, K, burnin)
        table = create_season_table(season, teams, metric)
        table['iteration'] = i
        dfs.append(table)
    return pd.concat(dfs, ignore_index=True)


def simulate_season(df, trace, metric='score', K=1, burnin=1000):
    """
...
def create_season_table(season, teams, metric='score'):
    '''
...
def predictions(df, simuls, teams, nsims=5000, metric='score'):
    ...
    if season_sim:
        df_observed = create_season_table(df, teams, metric)
    else:
