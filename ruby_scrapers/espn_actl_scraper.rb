...
while i<cids.length
  ...
  while j<weeks.length
    ...
    url = "http://games.espn.com/ffl/leaders?&slotCategoryId=#{cids[i]}&startIndex=0&scoringPeriodId=#{weeks[j]}&seasonId=#{year}"
    ...
    k = 0;
    doc.xpath('//tr//td[contains(concat(" ", @class, " "), " playertableStat appliedPoints appliedPointsProGameFinal ")]').each do |node|
      fpts[k] = node.text
      k += 1;
    end
